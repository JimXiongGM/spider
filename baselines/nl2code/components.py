...
from nn.layers.recurrent import LSTM, BiLSTM, CondAttLSTM
...
class PointerNet(Layer):
    def __init__(self, name="PointerNet"):
        ...
        self.dense1_input = Dense(
            config.encoder_hidden_dim, config.ptrnet_hidden_dim, activation="linear", name="Dense1_input"
        )
...
class CondAttLSTM(Layer):
    ...
    def __init__(
        ...
        self.params += [self.hatt_h_W1, self.hatt_hist_W1, self.hatt_b1, self.hatt_W2, self.hatt_b2]
    ...
    def _step(
        ...
        h_ctx_vec = T.switch(t, _attention_over_history(), T.zeros_like(h_tm1))
        ...
        par_h = T.switch(t, hist_h[T.arange(hist_h.shape[0]), parent_t, :], T.zeros_like(h_tm1))

        ##### feed in parent hidden state #####
        if config.tree_attention:
            i_t = self.inner_activation(
                xi_t
                + T.dot(h_tm1 * b_u[0], u_i)
                + T.dot(ctx_vec, c_i)
                + T.dot(par_h, p_i)
                + T.dot(h_ctx_vec, h_i)
            )
            ...
            i_t = self.inner_activation(
                xi_t + T.dot(h_tm1 * b_u[0], u_i) + T.dot(ctx_vec, c_i) + T.dot(par_h, p_i)
            )  # + T.dot(h_ctx_vec, h_i)
            f_t = self.inner_activation(
                xf_t + T.dot(h_tm1 * b_u[1], u_f) + T.dot(ctx_vec, c_f) + T.dot(par_h, p_f)
            )  # + T.dot(h_ctx_vec, h_f)
            c_t = f_t * c_tm1 + i_t * self.activation(
                xc_t + T.dot(h_tm1 * b_u[2], u_c) + T.dot(ctx_vec, c_c) + T.dot(par_h, p_c)
            )  # + T.dot(h_ctx_vec, h_c)
            o_t = self.inner_activation(
                xo_t + T.dot(h_tm1 * b_u[3], u_o) + T.dot(ctx_vec, c_o) + T.dot(par_h, p_o)
            )  # + T.dot(h_ctx_vec, h_o)
    ...
    def _for_step(
        ...
        c_t = f_t * c_tm1 + i_t * self.activation(
            xc_t + T.dot(h_tm1 * b_u[2], self.U_c) + T.dot(ctx_vec, self.C_c) + T.dot(h_ctx_vec, self.H_c)
        )
        o_t = self.inner_activation(
