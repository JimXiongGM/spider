...
from astnode import *
from components import CondAttLSTM, Hyp, PointerNet
...
class Model:
    def __init__(self, vocab, glove_path):
        ...
        if config.encoder == "bilstm":
            self.query_encoder_lstm = BiLSTM(
        ...
        self.decoder_lstm = CondAttLSTM(
            config.rule_embed_dim + config.node_embed_dim + config.rule_embed_dim,
    ...
    def build(self):
        ...
        tgt_par_rule_embed = T.switch(
            tgt_par_rule_seq[:, :, None] < 0,
            T.alloc(0.0, 1, config.rule_embed_dim),
        ...
        query_embed = self.query_encoder_lstm(
            query_token_embed, mask=query_token_embed_mask, dropout=config.dropout, srng=self.srng
        )
        ...
        logits = (
            T.dot(decoder_hidden_state_trans_token, T.transpose(self.vocab_embedding_W))
            + self.vocab_embedding_b
        )
        ...
        vocab_predict = softmax(
            ...
            + (T.min(logits.transpose(1, 0, 2), axis=1, keepdims=True) - 1) * (1 - mask)
        ).transpose(1, 0, 2)
        ...
        # (batch_size, max_example_action_num)
        tgt_prob = (
            tgt_action_seq_type[:, :, 0] * rule_tgt_prob
        ...
        likelihood = T.log(tgt_prob + 1.0e-7 * (1 - tgt_action_seq_mask))
    ...
    def build_decoder(self, query_tokens, query_token_embed, query_token_embed_mask, mask):
        ...
        query_embed = self.query_encoder_lstm(
            query_token_embed, mask=query_token_embed_mask, dropout=config.dropout, train=False
        )
        ...
        if not config.frontier_node_type_feed:
            node_embed_reshaped *= 0.0

        if not config.parent_action_feed:
            par_rule_embed_reshaped *= 0.0
        ...
        logits = (
            T.dot(decoder_next_state_trans_token, T.transpose(self.vocab_embedding_W))
            + self.vocab_embedding_b
        )
        # vocab_predict = softmax(T.dot(decoder_hidden_state_trans_token, T.transpose(self.vocab_embedding_W)) + self.vocab_embedding_b)
        ...
        outputs = [decoder_next_state, decoder_next_cell, rule_prob, gen_action_prob, vocab_prob, copy_prob]
    ...
    def decode(self, example, grammar, terminal_vocab, beam_size, max_time_step, log=False):
        ...
        for t in xrange(max_time_step):
            ...
            (
                ...
                vocab_prob,
                copy_prob,
            ) = self.decoder_func_next_step(*inputs)
    ...
    def pull_params(self):
        return OrderedDict(
            [(p_name, p.get_value(borrow=False)) for (p_name, p) in self.params_dict.iteritems()]
        )
    ...
    def load(self, model_file):
        ...
        for p_name, p in self.params_dict.iteritems():
            if p_name not in weights_dict:
                ...
                ), "shape mis-match for [%s]!, %s != %s" % (
                    p_name,
                    p.shape.eval(),
                    weights_dict[p_name].shape,
                )
