...
from tensorflow.python.ops.distributions import bernoulli, categorical
...
def _unstack_ta(inp):
    return tensor_array_ops.TensorArray(
        dtype=inp.dtype, size=array_ops.shape(inp)[0], element_shape=inp.get_shape()[1:]
    ).unstack(inp)
...
class ScheduledEmbeddingTrainingHelper(TrainingHelper):
    ...
    def __init__(
        ...
        """
        with ops.name_scope(name, "ScheduledEmbeddingSamplingWrapper", [embedding, sampling_probability]):
    ...
    def sample(self, time, outputs, state, name=None):
        with ops.name_scope(name, "ScheduledEmbeddingTrainingHelperSample", [time, outputs, state]):
            # Return -1s where we did not sample, and sample_ids elsewhere
            select_sample_noise = random_ops.random_uniform([self.batch_size], seed=self._scheduling_seed)
    ...
    def next_inputs(self, time, outputs, state, sample_ids, name=None):
        with ops.name_scope(
            name, "ScheduledEmbeddingTrainingHelperSample", [time, outputs, state, sample_ids]
        ):
            (finished, base_next_inputs, state) = super(ScheduledEmbeddingTrainingHelper, self).next_inputs(
                time=time, outputs=outputs, state=state, sample_ids=sample_ids, name=name
            )

            def maybe_sample():
                """Perform scheduled sampling."""
                where_sampling = math_ops.cast(array_ops.where(sample_ids > -1), dtypes.int32)
                where_not_sampling = math_ops.cast(array_ops.where(sample_ids <= -1), dtypes.int32)
                ...
                inputs_not_sampling = array_ops.gather(base_next_inputs, where_not_sampling_flat)
            ...
            all_finished = math_ops.reduce_all(finished)
            next_inputs = control_flow_ops.cond(all_finished, lambda: base_next_inputs, maybe_sample)
            return (finished, next_inputs, state)
...
class ScheduledOutputTrainingHelper(TrainingHelper):
    ...
    def __init__(
        ...
        """
        with ops.name_scope(
            name, "ScheduledOutputTrainingHelper", [inputs, auxiliary_inputs, sampling_probability]
        ):
            self._sampling_probability = ops.convert_to_tensor(
                sampling_probability, name="sampling_probability"
            )
            ...
            if auxiliary_inputs is None:
                ...
                inputs = ops.convert_to_tensor(inputs, name="inputs")
                auxiliary_inputs = ops.convert_to_tensor(auxiliary_inputs, name="auxiliary_inputs")
            ...
            self._auxiliary_input_tas = (
                nest.map_structure(_unstack_ta, auxiliary_inputs) if auxiliary_inputs is not None else None
            )

            self._seed = seed

            if next_input_layer is not None and not isinstance(
                next_input_layer, layers_base._Layer
    ...
    def sample(self, time, outputs, state, name=None):
        with ops.name_scope(name, "ScheduledOutputTrainingHelperSample", [time, outputs, state]):
            sampler = bernoulli.Bernoulli(probs=self._sampling_probability)
            return math_ops.cast(sampler.sample(sample_shape=self.batch_size, seed=self._seed), dtypes.bool)

    def next_inputs(self, time, outputs, state, sample_ids, name=None):
        with ops.name_scope(
            name, "ScheduledOutputTrainingHelperNextInputs", [time, outputs, state, sample_ids]
        ):
            ...
            def maybe_sample():
                ...
                def maybe_concatenate_auxiliary_inputs(outputs_, indices=None):
                    ...
                    return nest.map_structure(
                        lambda x, y: array_ops.concat((x, y), -1), outputs_, auxiliary_inputs
                    )

                if self._next_input_layer is None:
                    return array_ops.where(
                        sample_ids, maybe_concatenate_auxiliary_inputs(outputs), base_next_inputs
                    )
                ...
                inputs_not_sampling = array_ops.gather_nd(base_next_inputs, where_not_sampling)
