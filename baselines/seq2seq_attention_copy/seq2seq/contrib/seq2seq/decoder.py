...
from tensorflow.python.ops import array_ops, control_flow_ops, math_ops, tensor_array_ops, variable_scope
...
def _transpose_batch_time(x):
    ...
    x_t.set_shape(
        tensor_shape.TensorShape([x_static_shape[1].value, x_static_shape[0].value]).concatenate(
            x_static_shape[2:]
        )
    )
    return x_t
    ...
    def _t(s):
        return (
            ...
            if isinstance(s, ops.Tensor)
                ...
                tensor_shape.TensorShape(s).as_list(), dtype=dtypes.int32, name="zero_suffix_shape"
            )
        )
        ...
        def body(time, outputs_ta, state, inputs, finished):
            ...
            if impute_finished:
                emit = nest.map_structure(
                    lambda out, zero: array_ops.where(finished, zero, out), next_outputs, zero_outputs
                )
            else:
                emit = next_outputs
