...
class Seq2SeqModel(ModelBase):
    ...
    def _clip_gradients(self, grads_and_vars):
        ...
        for gradient, variable in grads_and_vars:
            if "embedding" in variable.name:
                ...
                tmp = tf.clip_by_norm(gradient.values, self.params["optimizer.clip_embed_gradients"])
    ...
    def _create_predictions(self, decoder_output, features, labels, losses=None):
        ...
        decoder_output_flat = {k: _transpose_batch_time(v) for k, v in decoder_output_flat.items()}
    ...
    def source_embedding(self):
        ...
        if self.embedding_mat_source is not None:
            ...
            initializer = tf.random_uniform_initializer(
                -self.params["embedding.init_scale"], self.params["embedding.init_scale"], dtype=tf.float32
            )
    ...
    def _get_beam_search_decoder(self, decoder):
        ...
        config = beam_search.BeamSearchConfig(
            ...
            choose_successors_fn=getattr(
                beam_search, self.params["inference.beam_search.choose_successors_fn"]
            ),
        )
    ...
    def _preprocess(self, features, labels):
        ...
        if self.params["source.max_seq_len"] is not None:
            features["source_tokens"] = features["source_tokens"][:, : self.params["source.max_seq_len"]]
            features["source_len"] = tf.minimum(features["source_len"], self.params["source.max_seq_len"])
    ...
    def _build(self, features, labels, params):
        ...
        (
            decoder_output,
            _,
        ) = self.decode(encoder_output, features, labels)
