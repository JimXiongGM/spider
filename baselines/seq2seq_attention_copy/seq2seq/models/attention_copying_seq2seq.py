...
class BaseAttentionCopyingSeq2Seq(AttentionSeq2Seq):
    ...
    def _create_decoder(self, encoder_output, features, _labels):
        ...
        attention_class = locate(self.params["attention.class"]) or getattr(
            decoders.attention, self.params["attention.class"]
        )
        attention_layer = attention_class(
            params=self.params["attention.params"], mode=self.mode, name="attention_to_input_layer"
        )
        ...
        if self.use_beam_search:
            reverse_scores_lengths = tf.tile(
                input=reverse_scores_lengths, multiples=[self.params["inference.beam_search.beam_width"]]
            )

        if "schema" in self.params["decoder.class"].lower():
            schema_attention_class = locate(self.params["schema.attention.class"]) or getattr(
                decoders.attention, self.params["schema.attention.class"]
            )
    ...
    def _trim_copy_indices(self, labels, indices_name):
        ...
        if self.params["target.max_seq_len"] is not None:
            labels[indices_name] = labels[indices_name][:, : self.params["target.max_seq_len"]]
        return labels
    ...
    def _set_special_vocab_ids(self):
        ...
        self.copy_word_id = target_vocab_to_id.lookup(tf.convert_to_tensor("COPY_WORD", dtype=tf.string))
        self.copy_schema_id = target_vocab_to_id.lookup(tf.convert_to_tensor("COPY_SCHEMA", dtype=tf.string))
    ...
    def _get_predicted_tokens(self, predictions):
        if "predicted_tokens" in predictions.keys():
            ...
            output_predicted_tokens = target_id_to_vocab.lookup(tf.to_int64(predictions["predicted_ids"]))
        return output_predicted_tokens

    def _fill_in_copies(self, prediction_ids, predicted_tokens, attn_scores, copy_source_tokens, copy_id):
        predicted_copy_indices = math_ops.cast(math_ops.argmax(attn_scores, axis=-1), tf.int32)
        ...
        row_nums_3d = tf.expand_dims(
            tf.tile(
                tf.expand_dims(tf.range(0, limit=tf.shape(predicted_copy_indices)[0]), axis=1),
                [1, tf.shape(predicted_copy_indices)[1]],
            ),
            2,
        )
        ...
        predicted_tokens = tf.where(copy_map, copy_predicted_tokens, predicted_tokens)
        return predicted_tokens

    def _copy_loss(self, targets, seq_len, attention_scores, copy_indices, copy_id):
    ...
    def compute_loss(self, decoder_output, _features, labels):
        ...
        seq_loss = seq2seq_losses.cross_entropy_sequence_loss(
            logits=decoder_output.logits[:, :, :], targets=targets, sequence_length=seq_len
        )
        return seq_loss
...
class SchemaAttentionCopyingSeq2Seq(BaseAttentionCopyingSeq2Seq):
    ...
    def _preprocess(self, features, labels):
        ...
        """
        features, labels = super(SchemaAttentionCopyingSeq2Seq, self)._preprocess(features, labels)
    ...
    def _create_predictions(self, decoder_output, features, labels, losses=None):
        predictions = super(SchemaAttentionCopyingSeq2Seq, self)._create_predictions(
            decoder_output, features, labels, losses
        )
        if "predicted_ids" in predictions.keys():
            ...
            schema_tokens = tf.sparse_tensor_to_dense(
                tf.string_split(schema_tokens), default_value="UNK", name="desparsifying_schema_tokens"
            )

            # Figure out copy_schema_predicted_tokens
            predicted_tokens = self._fill_in_copies(
                prediction_ids,
                output_predicted_tokens,
    ...
    def compute_loss(self, decoder_output, _features, labels):
        ...
        losses and loss is a single scalar tensor to minimize.
        """
        seq_loss = super(SchemaAttentionCopyingSeq2Seq, self).compute_loss(decoder_output, _features, labels)

        targets, seq_length = self._targets_and_seq_len(labels)
        schema_copy_loss = self._copy_loss(
            targets,
            seq_length,
            decoder_output.schema_attention_copy_vals,
        ...
        loss = tf.reduce_sum(losses) / tf.to_float(tf.reduce_sum(labels["target_len"] - 1))
...
class InputAttentionCopyingSeq2Seq(BaseAttentionCopyingSeq2Seq):
    ...
    def _preprocess(self, features, labels):
        ...
        - Trims copy indices to target.max_seq_len
        """
        features, labels = super(InputAttentionCopyingSeq2Seq, self)._preprocess(features, labels)
    ...
    def _create_predictions(self, decoder_output, features, labels, losses=None):
        predictions = super(InputAttentionCopyingSeq2Seq, self)._create_predictions(
            decoder_output, features, labels, losses
        )
    ...
    def compute_loss(self, decoder_output, _features, labels):
        ...
        losses and loss is a single scalar tensor to minimize.
        """
        seq_loss = super(InputAttentionCopyingSeq2Seq, self).compute_loss(decoder_output, _features, labels)
        targets, seq_length = self._targets_and_seq_len(labels)
        word_copy_loss = self._copy_loss(
            targets,
            seq_length,
...
class SchemaAndInputAttentionCopyingSeq2Seq(SchemaAttentionCopyingSeq2Seq):
    ...
    def __init__(self, params, mode, name="schema_att_copying_seq2seq"):
        super(SchemaAndInputAttentionCopyingSeq2Seq, self).__init__(params, mode, name)
        self.copy_word_id = None
    ...
    def _preprocess(self, features, labels):
        ...
        features, labels = super(SchemaAndInputAttentionCopyingSeq2Seq, self)._preprocess(features, labels)
    ...
    def _create_predictions(self, decoder_output, features, labels, losses=None):
        predictions = super(SchemaAndInputAttentionCopyingSeq2Seq, self)._create_predictions(
            decoder_output, features, labels, losses
        )
        if "predicted_ids" in predictions.keys():
            prediction_ids = predictions["predicted_ids"]
    ...
    def compute_loss(self, decoder_output, _features, labels):
        ...
        losses and loss is a single scalar tensor to minimize.
        """
        seq_and_schema_loss, _ = super(SchemaAndInputAttentionCopyingSeq2Seq, self).compute_loss(
