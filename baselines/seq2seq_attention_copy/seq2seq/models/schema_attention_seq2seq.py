...
class SchemaAttentionSeq2Seq(BasicSeq2Seq):
    ...
    def _schema_lookups(self, features):
        ...
        return (schema_embeddings_3d, schema_embeddings_3d, schema_attn_values_length)
    ...
    def _get_decoder_args(self, encoder_output, features, _labels):
        ...
        attention_layer = attention_class(params=self.params["attention.params"], mode=self.mode)
    ...
    def _create_decoder(self, encoder_output, features, _labels):
        (
            ...
            reverse_scores_lengths,
            schema_attention_fn,
        ) = self._get_decoder_args(encoder_output, features, _labels)
        (
            schema_attention_keys,
            schema_attention_values,
            schema_attention_values_length,
        ) = self._schema_lookups(features)
...
class SchemaMapAttentionSeq2Seq(SchemaAttentionSeq2Seq):
    ...
    def _schema_lookups(self, features):
        ...
        schema_map_attn_values_length = tf.squeeze(tf.gather(schema_map_lengths, ids), [1])

        return (
            schema_embeddings_3d,
            schema_embeddings_3d,
            schema_attn_values_length,
            schema_maps_3d,
            schema_maps_3d,
            schema_map_attn_values_length,
        )
    ...
    def _create_decoder(self, encoder_output, features, _labels):
        ...
        (
            ...
            schema_attention_values,
            schema_attention_values_length,
            schema_map_attention_keys,
            schema_map_attention_values,
            schema_map_attention_values_length,
        ) = self._schema_lookups(features)

        schema_map_attention_class = locate(self.params["schema_map.attention.class"]) or getattr(
            decoders.attention, self.params["schema.attention.class"]
        )
