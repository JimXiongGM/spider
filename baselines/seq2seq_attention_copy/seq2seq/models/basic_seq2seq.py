...
class BasicSeq2Seq(Seq2SeqModel):
    ...
    def _decode_train(
        self, decoder, bridge, _encoder_output, _features, labels
    ):  # where are the labels from?
        """Runs decoding in training mode"""
        target_embedded = tf.nn.embedding_lookup(self.target_embedding, labels["target_ids"])
        helper_train = tf_decode_helper.TrainingHelper(
            inputs=target_embedded[:, :-1], sequence_length=labels["target_len"] - 1
        )
        decoder_initial_state = bridge()
        return decoder(decoder_initial_state, helper_train)

    # todo:check
    def _decode_infer(self, decoder, bridge, _encoder_output, features, labels, mask=None):
    ...
    def encode(self, features, labels):  # what is the features?
        source_embedded = tf.nn.embedding_lookup(self.source_embedding, features["source_ids"])
    ...
    def decode(self, encoder_output, features, labels):
        ...
        bridge = self._create_bridge(
            encoder_outputs=encoder_output, decoder_state_size=decoder.cell.state_size
        )
        if self.mode == tf.contrib.learn.ModeKeys.INFER:
            return self._decode_infer(decoder, bridge, encoder_output, features, labels)
        else:
