...
def position_encoding(sentence_size, embedding_size):
    ...
    for k in range(1, le):
        for j in range(1, ls):
            encoding[j - 1, k - 1] = (1.0 - j / float(ls)) - (k / float(le)) * (1.0 - 2.0 * j / float(ls))
    return encoding
...
def _create_position_embedding(embedding_dim, num_positions, lengths, maxlen):
    ...
    position_encodings = tf.constant(
        position_encoding(num_positions, embedding_dim), name="position_encoding"
    )
...
class PoolingEncoder(Encoder):
    ...
    def encode(self, inputs, sequence_length):
        ...
        outputs = self._pooling_fn(
            inputs=inputs, pool_size=self.params["pool_size"], strides=self.params["strides"], padding="SAME"
        )
