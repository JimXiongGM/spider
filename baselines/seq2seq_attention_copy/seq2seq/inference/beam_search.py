...
def length_penalty(sequence_lengths, penalty_factor):
    ...
    """
    return tf.div((5.0 + tf.to_float(sequence_lengths)) ** penalty_factor, (5.0 + 1.0) ** penalty_factor)
...
def mask_probs(probs, eos_token, finished):
    ...
    finished_mask = tf.expand_dims(tf.to_float(1.0 - tf.to_float(finished)), 1)
    # These examples are not finished and we leave them
    non_finished_examples = finished_mask * probs
    # All finished examples are replaced with a vector that has all
    # probability on EOS
    finished_row = tf.one_hot(eos_token, vocab_size, dtype=tf.float32, on_value=0.0, off_value=tf.float32.min)
    finished_examples = (1.0 - finished_mask) * finished_row
