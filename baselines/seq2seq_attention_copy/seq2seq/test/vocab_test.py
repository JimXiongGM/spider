...
class CreateVocabularyLookupTableTest(tf.test.TestCase):
    ...
    def test_without_counts(self):
        ...
        vocab_to_id_table, id_to_vocab_table, _, vocab_size = vocab.create_vocabulary_lookup_table(
            vocab_file.name
        )
        ...
        with self.test_session() as sess:
            ...
            np.testing.assert_array_equal(
                np.char.decode(words.astype("S"), "utf-8"), ["Hello", ".", "笑", "UNK"]
            )

    def test_with_counts(self):
        vocab_list = ["Hello", ".", "笑"]
        vocab_counts = [100, 200, 300]
        vocab_file = test_utils.create_temporary_vocab_file(vocab_list, vocab_counts)
