...
from seq2seq.models import AttentionSeq2Seq, BasicSeq2Seq
...
class EncoderDecoderTests(tf.test.TestCase):
    ...
    def _test_pipeline(self, mode, params=None):
        ...
        input_pipeline_ = input_pipeline.ParallelTextInputPipeline(
            params={"source_files": [sources_file.name], "target_files": [targets_file.name]}, mode=mode
        )
    ...
    def test_train(self):
        ...
        np.testing.assert_array_equal(
            ...
            [self.batch_size, expected_decode_len - 1, model.target_vocab_info.total_size],
        )
        np.testing.assert_array_equal(
            predictions_["losses"].shape, [self.batch_size, expected_decode_len - 1]
        )
        np.testing.assert_array_equal(
            predictions_["predicted_ids"].shape, [self.batch_size, expected_decode_len - 1]
        )
    ...
    def test_infer(self):
        ...
        np.testing.assert_array_equal(
            predictions_["logits"].shape, [self.batch_size, pred_len, model.target_vocab_info.total_size]
        )
    ...
    def test_infer_beam_search(self):
        ...
        model, fetches_ = self._test_pipeline(
            mode=tf.contrib.learn.ModeKeys.INFER, params={"inference.beam_search.beam_width": 10}
        )
        (predictions_,) = fetches_
        pred_len = predictions_["predicted_ids"].shape[1]
        ...
        np.testing.assert_array_equal(
            predictions_["beam_search_output.beam_parent_ids"].shape, [1, pred_len, beam_width]
        )
        np.testing.assert_array_equal(
            predictions_["beam_search_output.scores"].shape, [1, pred_len, beam_width]
        )
        np.testing.assert_array_equal(
            predictions_["beam_search_output.original_outputs.predicted_ids"].shape, [1, pred_len, beam_width]
        )
        np.testing.assert_array_equal(
