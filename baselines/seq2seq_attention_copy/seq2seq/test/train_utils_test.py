...
class TestGetRNNCell(tf.test.TestCase):
    ...
    def test_full_class_path(self):
        cell = training_utils.get_rnn_cell(
            cell_class="tensorflow.contrib.rnn.BasicRNNCell", cell_params={"num_units": 16}, num_layers=1
        )
    ...
    def test_extra_args(self):
        ...
        with self.assertRaises(ValueError):
            training_utils.get_rnn_cell(
                cell_class="LSTMCell", cell_params={"num_units": 16, "use_peepholesERROR": True}, num_layers=1
            )
...
class TestTrainOptions(tf.test.TestCase):
    ...
    def test_read_write(self):
        saved_opts = training_utils.TrainOptions(model_class=self.model_class, model_params=self.model_params)
...
class TestInputFn(tf.test.TestCase):
    ...
    def _test_with_args(self, **kwargs):
        ...
        pipeline = input_pipeline.ParallelTextInputPipeline(
            params={"source_files": [sources_file.name], "target_files": [targets_file.name]},
...
class TestLRDecay(tf.test.TestCase):
    ...
    def test_no_decay(self):
        ...
        decay_fn = training_utils.create_learning_rate_decay_fn(decay_type="", decay_steps=5, decay_rate=2.0)
        self.assertEqual(decay_fn, None)
    ...
    def test_decay_without_min(self):
        ...
        with self.test_session() as sess:
            ...
            np.testing.assert_almost_equal(
                sess.run(decay_fn(initial_lr, 5000)), initial_lr * 0.9 ** ((1000.0 - 100.0) / 10.0)
            )
