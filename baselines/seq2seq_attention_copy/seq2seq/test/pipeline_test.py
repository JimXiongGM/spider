...
class PipelineTest(tf.test.TestCase):
    ...
    def test_train_infer(self):
        ...
        sources_dev, targets_dev = test_utils.create_temp_parallel_data(
            sources=["a a", "b b", "c c c", "笑 笑 笑"], targets=["b b", "a a", "c c c", "泣 泣 泣"]
        )
        ...
        train_script = imp.load_source("seq2seq.test.train_bin", os.path.join(BIN_FOLDER, "train.py"))
    ...
    vocab_target: {}
    """.format(
            vocab_source.name, vocab_target.name
        )
            ...
            yaml.dump(
                {
                    ...
                    "model_params": {
                        "embedding.dim": 10,
                        "decoder.params": {
                            "rnn_cell": {"cell_class": "GRUCell", "cell_params": {"num_units": 8}}
                        },
                        "encoder.params": {
                            "rnn_cell": {"cell_class": "GRUCell", "cell_params": {"num_units": 8}}
                        },
        ...
        expected_checkpoint = os.path.join(self.output_dir, "model.ckpt-50.data-00000-of-00001")
        self.assertTrue(os.path.exists(expected_checkpoint))

        # Reset flags and import inference script
        _clear_flags()
        tf.reset_default_graph()
        infer_script = imp.load_source("seq2seq.test.infer_bin", os.path.join(BIN_FOLDER, "infer.py"))
      ...
      params:
        ...
        target_files:
          - {}
    """.format(
            sources_dev.name, targets_dev.name
        )
        tf.app.flags.FLAGS.batch_size = 2
        tf.app.flags.FLAGS.checkpoint_path = os.path.join(self.output_dir, "model.ckpt-50")
    ...
    """.format(
        ...
        self.assertTrue(os.path.exists(os.path.join(attention_dir, "attention_scores.npz")))
    ...
    - class: DumpBeams
      params:
        file: {}
    """.format(
            os.path.join(self.output_dir, "beams.npz")
        )
