...
from seq2seq.decoders import AttentionDecoder, AttentionLayerDot, BasicDecoder, beam_search_decoder
...
class DecoderTests(object):
    ...
    def test_with_fixed_inputs(self):
        ...
        decoder_fn = self.create_decoder(helper=helper, mode=tf.contrib.learn.ModeKeys.TRAIN)
        ...
        np.testing.assert_array_equal(
            decoder_output_.logits.shape, [self.sequence_length, self.batch_size, self.vocab_size]
        )
        np.testing.assert_array_equal(
            decoder_output_.predicted_ids.shape, [self.sequence_length, self.batch_size]
        )
    ...
    def test_gradients(self):
        ...
        losses = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=decoder_output.logits, labels=labels)
    ...
    def test_with_dynamic_inputs(self):
        ...
        decoder_fn = self.create_decoder(helper=helper, mode=tf.contrib.learn.ModeKeys.INFER)
        initial_state = decoder_fn.cell.zero_state(self.batch_size, dtype=tf.float32)
        decoder_output, _ = decoder_fn(initial_state, helper)
        ...
        np.testing.assert_array_equal(
            decoder_output_.logits.shape, [self.max_decode_length, self.batch_size, self.vocab_size]
        )
        np.testing.assert_array_equal(
            decoder_output_.predicted_ids.shape, [self.max_decode_length, self.batch_size]
        )
    ...
    def test_with_beam_search(self):
        ...
        decoder_fn = self.create_decoder(helper=helper, mode=tf.contrib.learn.ModeKeys.INFER)
        decoder_fn = beam_search_decoder.BeamSearchDecoder(decoder=decoder_fn, config=config)
        ...
        np.testing.assert_array_equal(
            decoder_output_.predicted_ids.shape, [self.max_decode_length, 1, config.beam_width]
        )
        np.testing.assert_array_equal(
        ...
        np.testing.assert_array_equal(
            decoder_output_.beam_search_output.scores.shape, [self.max_decode_length, 1, config.beam_width]
        )
        np.testing.assert_array_equal(
...
class AttentionDecoderTest(tf.test.TestCase, DecoderTests):
    ...
    def test_attention_scores(self):
        ...
        np.testing.assert_array_almost_equal(scores_sum, np.ones([self.sequence_length, self.batch_size]))
