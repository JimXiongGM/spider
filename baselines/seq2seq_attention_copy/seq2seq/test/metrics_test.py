...
from seq2seq.metrics import bleu, rouge
from seq2seq.metrics.metric_specs import BleuMetricSpec, RougeMetricSpec
...
class TestMosesBleu(tf.test.TestCase):
    ...
    def _test_multi_bleu(self, hypotheses, references, lowercase, expected_bleu):
        ...
        result = bleu.moses_multi_bleu(hypotheses=hypotheses, references=references, lowercase=lowercase)
    ...
    def _test_metric_spec(self, metric_spec, hyps, refs, expected_scores):
        ...
        with self.test_session() as sess:
            ...
            for hyp, ref in zip(hyps, refs):
                hyp = hyp.split(" ")
                ref = ref.split(" ")
                sess.run(update_op, {predictions["predicted_tokens"]: [hyp], labels["target_tokens"]: [ref]})
