...
class UnidirectionalRNNEncoderTest(tf.test.TestCase):
    ...
    def test_encode(self):
        ...
        np.testing.assert_array_equal(
            encoder_output_.outputs.shape, [self.batch_size, self.sequence_length, 32]
        )
        self.assertIsInstance(encoder_output_.final_state, tf.contrib.rnn.LSTMStateTuple)
        np.testing.assert_array_equal(encoder_output_.final_state.h.shape, [self.batch_size, 32])
        np.testing.assert_array_equal(encoder_output_.final_state.c.shape, [self.batch_size, 32])
...
class BidirectionalRNNEncoderTest(tf.test.TestCase):
    ...
    def test_encode(self):
        ...
        np.testing.assert_array_equal(
            encoder_output_.outputs.shape, [self.batch_size, self.sequence_length, 32 * 2]
        )
        ...
        np.testing.assert_array_equal(encoder_output_.final_state[0].h.shape, [self.batch_size, 32])
        np.testing.assert_array_equal(encoder_output_.final_state[0].c.shape, [self.batch_size, 32])
        np.testing.assert_array_equal(encoder_output_.final_state[1].h.shape, [self.batch_size, 32])
        np.testing.assert_array_equal(encoder_output_.final_state[1].c.shape, [self.batch_size, 32])
...
class StackBidirectionalRNNEncoderTest(tf.test.TestCase):
    ...
    def _test_encode_with_params(self, params):
        ...
        np.testing.assert_array_equal(
            encoder_output_.outputs.shape, [self.batch_size, self.sequence_length, output_size * 2]
        )
    ...
    def test_encode_with_single_cell(self):
        ...
        np.testing.assert_array_equal(encoder_output_.final_state[0][0].h.shape, [self.batch_size, 32])
        np.testing.assert_array_equal(encoder_output_.final_state[0][0].c.shape, [self.batch_size, 32])
        np.testing.assert_array_equal(encoder_output_.final_state[1][0].h.shape, [self.batch_size, 32])
        np.testing.assert_array_equal(encoder_output_.final_state[1][0].c.shape, [self.batch_size, 32])
    ...
    def test_encode_with_multi_cell(self):
        ...
        for layer_idx in range(4):
            self.assertIsInstance(encoder_output_.final_state[0][layer_idx], tf.contrib.rnn.LSTMStateTuple)
            self.assertIsInstance(encoder_output_.final_state[1][layer_idx], tf.contrib.rnn.LSTMStateTuple)
            np.testing.assert_array_equal(
                encoder_output_.final_state[0][layer_idx].h.shape, [self.batch_size, 32]
            )
            np.testing.assert_array_equal(
                encoder_output_.final_state[0][layer_idx].c.shape, [self.batch_size, 32]
            )
            np.testing.assert_array_equal(
                encoder_output_.final_state[1][layer_idx].h.shape, [self.batch_size, 32]
            )
            np.testing.assert_array_equal(
                encoder_output_.final_state[1][layer_idx].c.shape, [self.batch_size, 32]
            )
