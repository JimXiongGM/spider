...
from tensorflow.contrib.slim.python.slim.data import data_provider, parallel_reader
...
def _make_copying_data_provider_base(
    data_sources_source,
    data_sources_schema,
    ...
    **kwargs
    ...
    if data_sources_schema is not None:
        decoder_schemas = split_tokens_decoder.SplitMaskDecoder(
            decoder_mask_feature_name="decoder_mask", delimiter=" "
        )
...
def make_schema_copying_data_provider(
    ...
    **kwargs
    ...
    return SchemaCopyingDataProvider(
        dataset1=dataset_source, dataset2=dataset_target, schemas=dataset_schemas, **kwargs
    )


def make_schema_and_word_copying_data_provider(
    data_sources_source,
    data_sources_target,
    data_sources_schema,
    reader=tf.TextLineReader,
...
def make_word_copying_data_provider(
    data_sources_source,
    data_sources_target,
