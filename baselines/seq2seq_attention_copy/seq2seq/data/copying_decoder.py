...
class BaseCopyingDecoder(split_tokens_decoder.SplitTokensDecoder):
    ...
    def decode(self, data, items):
        ...
        if not isinstance(data, list):
            raise ValueError(
                "'data' arg to decode should be a three-item list, but is a %s" % str(type(data))
            )
        if len(data) != 3:
            raise ValueError(
                "'data' arg to decode should be a three-item list, but is a list of length %d" % len(data)
            )
    ...
    def _mark_copies(self, tokenized, copy_source, copy_token):
        ...
        tokenized = tf.where(
            tf_copy_result[0],  # bools from copy_fn
...
class SchemaAndWordCopyingDecoder(BaseCopyingDecoder):
    ...
    def _prepend(self, tokens, indices):
        ...
        input_copies_indices = tf.concat(
            [[0], input_copies_indices], 0, name="prepend_to_input_copies_indices"
        )
        schema_copies_indices = tf.concat(
            [[0], schema_copies_indices], 0, name="prepend_to_schema_copies_indices"
        )
    ...
    def _append(self, tokens, indices):
        ...
        input_copies_indices = indices[0]
        input_copies_indices = tf.concat(
            [input_copies_indices, [0]], 0, name="append_to_input_copies_indices"
        )
        schema_copies_indices = tf.concat(
            [schema_copies_indices, [0]], 0, name="append_to_schema_copies_indices"
        )
        return tokens, [input_copies_indices, schema_copies_indices]
