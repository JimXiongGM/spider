...
class TrainOptions(object):
    ...
    def load(model_dir):
        ...
        return TrainOptions(
            model_class=options_dict["model_class"], model_params=options_dict["model_params"]
        )
...
def cell_from_spec(cell_classname, cell_params):
    ...
    for key in cell_params.keys():
        if key not in cell_args:
          ...
          are: {}""".format(
                    key, cell_class.__name__, cell_args
                )
            )
    ...
    for _ in range(num_layers):
        ...
        if dropout_input_keep_prob < 1.0 or dropout_output_keep_prob < 1.0:
            cell = tf.contrib.rnn.DropoutWrapper(
                cell=cell, input_keep_prob=dropout_input_keep_prob, output_keep_prob=dropout_output_keep_prob
            )
        cells.append(cell)
...
def create_learning_rate_decay_fn(
    decay_type,
    decay_steps,
    decay_rate,
    start_decay_at=0,
    ...
    def decay_fn(learning_rate, global_step):
        ...
        final_lr = tf.train.piecewise_constant(
            x=global_step, boundaries=[start_decay_at], values=[learning_rate, decayed_learning_rate]
        )
...
def create_input_fn(
    pipeline, batch_size, bucket_boundaries=None, allow_smaller_final_batch=False, scope=None
):
