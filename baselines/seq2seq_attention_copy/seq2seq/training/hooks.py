...
import yaml
from seq2seq import global_vars, graph_utils
...
class TrainSampleHook(TrainingHook):
    ...
    @staticmethod
    def default_params():
        return {"every_n_secs": None, "every_n_steps": 1000, "source_delimiter": " ", "target_delimiter": " "}
    ...
    def after_run(self, _run_context, run_values):
        ...
        for result in result_dicts:
            ...
            result_str += self._target_delimiter.encode("utf-8").join(predicted_slice).decode("utf-8") + "\n"
...
class SyncReplicasOptimizerHook(TrainingHook):
    ...
    def begin(self):
        ...
        if self._sync_optimizer._gradients_applied is False:  # pylint: disable=protected-access
            raise ValueError(
                "SyncReplicasOptimizer.apply_gradient should be called before using " "the hook."
            )
        if self.is_chief:
    ...
    def after_create_session(self, session, coord):
        ...
        local_init_success, msg = session_manager._ready(  # pylint: disable=protected-access
            self._ready_for_local_init_op, session, "Model is not ready for SyncReplicasOptimizer local init."
        )
