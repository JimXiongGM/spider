...
class AttentionDecoderOutput(
    namedtuple(
        "DecoderOutput", ["logits", "predicted_ids", "cell_output", "attention_scores", "attention_context"]
    )
):
...
class AttentionDecoder(RNNDecoder):
    ...
    def step(self, time_, inputs, state, name=None):
        ...
        if self.mode == tf.contrib.learn.ModeKeys.INFER:
            ...
            sample_ids = self.helper.sample(time=time_, outputs=pred, state=cell_state)
        else:
