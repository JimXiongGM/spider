...
class SchemaAttentionDecoder(RNNDecoder):
    ...
    def initialize(self, name=None):
        ...
        attention_context = tf.zeros(
            [tf.shape(first_inputs)[0], self.attention_values.get_shape().as_list()[-1]]
        )
        schema_attention_context = tf.zeros(
            [tf.shape(first_inputs)[0], self.schema_attention_values.get_shape().as_list()[-1]]
        )
    ...
    def _setup(self, initial_state, helper):
        ...
        def att_next_inputs(time, outputs, state, sample_ids, name=None):
            ...
            next_inputs = tf.concat(
                [next_inputs, outputs.attention_context, outputs.schema_attention_context], 1
            )
...
class SchemaAttentionCopyingDecoder(SchemaAttentionDecoder):
    ...
    def compute_output(self, cell_output):
        (
            ...
            schema_att_scores,
            schema_attention_context,
        ) = super(SchemaAttentionCopyingDecoder, self).compute_output(cell_output)
        ...
        concatenated = tf.expand_dims(tf.concat([cell_output, attention_context], 1), axis=2)
...
class SchemaMapAttentionDecoder(SchemaAttentionDecoder):
    ...
    def initialize(self, name=None):
        (finished, first_inputs, initial_state) = super(SchemaMapAttentionDecoder, self).initialize(name=name)
    ...
    def compute_output(self, cell_output, calculate_softmax=True):
        (
            ...
            schema_att_scores,
            schema_attention_context,
        ) = super(SchemaMapAttentionDecoder, self).compute_output(cell_output, calculate_softmax=False)
        with tf.variable_scope("schema_map_att"):
            (schema_map_att_scores, schema_map_attention_context) = self.schema_map_attention_fn(
                query=cell_output,
        ...
        if calculate_softmax:
            softmax_input, logits = self._calculate_softmax(
                [cell_output, attention_context, schema_attention_context, schema_map_attention_context]
            )
        ...
        def att_next_inputs(time, outputs, state, sample_ids, name=None):
            ...
            next_inputs = tf.concat(
                [
                    ...
                    outputs.schema_map_attention_context,
                ],
                1,
            )
            return (finished, next_inputs, next_state)
